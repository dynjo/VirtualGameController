    ...
    open class VgcMotionManager: NSObject {
        ...
        func startPolling() {
            if self.active {
                ...
                self.motionPollingTimer = Timer.scheduledTimer(timeInterval: self.updateInterval, target: self, selector: #selector(self.pollMotionData), userInfo: nil, repeats: true)
                self.active = true
            }
        }
        ...
        @objc open func start() {
                ...
                if deviceIsTypeOfBridge() || self.deviceSupportsMotion == true {
                    ...
                    self.manager.startAccelerometerUpdates(to: OperationQueue.main, withHandler: { (accelerometerData, _) -> Void in
        ...
        @objc open func stop() {
                ...
                if self.active == true {
                    self.motionPollingTimer.invalidate()
                    self.active = false
                }
            #endif
        }
        ...
        @objc func sendElementState(_ element: Element) {
                ...
                if deviceIsTypeOfBridge() {
                    self.controller.peripheral.sendElementState(element)
                } else {
            ...
            #if os(watchOS)
                self.watchConnectivity.sendElementState(element: element)
            #endif
        }
